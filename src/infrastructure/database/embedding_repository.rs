...
pub(crate) fn decode_vector(bytes: &[u8]) -> Option<Vec<f32>> {
    ...
    Some(bytes.chunks_exact(4).map(|chunk| f32::from_le_bytes([chunk[0], chunk[1], chunk[2], chunk[3]])).collect())
}

fn storage_error(error: rusqlite::Error) -> EmbeddingRepositoryError { EmbeddingRepositoryError::Storage(Box::new(error)) }

impl fmt::Debug for SqliteEmbeddingRepository { fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.debug_struct("SqliteEmbeddingRepository").finish_non_exhaustive() } }
