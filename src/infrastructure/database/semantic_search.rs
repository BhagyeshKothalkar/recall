...
use crate::{
    application::ports::{SearchError, SemanticMemorySearcher, TextEmbedder},
    domain::{
        InferenceModel, Memory, MemoryId, MemorySource, SearchQuery, SearchResult, SearchScore,
        Timestamp,
    },
};
...
impl<E: TextEmbedder> SemanticMemorySearcher for SqliteSemanticMemorySearcher<E> {
    fn search_semantic(&self, query: &SearchQuery) -> Result<Vec<SearchResult>, SearchError> {
        let query_vector = self
            .embedder
            .embed_text(query.text(), &self.model)
            .map_err(|error| SearchError::Retrieval(Box::new(error)))?;
        ...
        let rows = statement
            .query_map(params![self.model.name()], |row| {
                let vector: Vec<u8> = row.get(6)?;
                let vector = decode_vector(&vector).ok_or_else(|| {
                ...
                let score = SearchScore::new((similarity + 1.0) / 2.0).ok_or_else(|| {
                    rusqlite::Error::InvalidColumnType(
                        6,
                        "score".into(),
...
fn read_memory(row: &rusqlite::Row<'_>) -> rusqlite::Result<Memory> {
    ...
    Memory::new(
        ...
        row.get(1)?,
        source,
        Timestamp::from_unix_millis(row.get(4)?),
...
impl<E> fmt::Debug for SqliteSemanticMemorySearcher<E> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("SqliteSemanticMemorySearcher")
...
mod tests {
    ...
    impl TextEmbedder for FakeEmbedder {
        ...
        ) -> Result<Vec<f32>, crate::application::ports::InferenceError> {
